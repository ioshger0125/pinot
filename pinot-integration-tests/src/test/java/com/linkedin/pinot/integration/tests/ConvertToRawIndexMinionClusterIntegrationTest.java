...
/**
 * Integration test that extends HybridClusterIntegrationTest and add Minions into the cluster to convert all metric
...
public class ConvertToRawIndexMinionClusterIntegrationTest extends HybridClusterIntegrationTest {
  ...
  protected TableTaskConfig getTaskConfig() {
    ...
    taskTypeConfigsMap.put(MinionConstants.ConvertToRawIndexTask.TASK_TYPE,
        Collections.singletonMap(MinionConstants.TABLE_MAX_NUM_TASKS_KEY, "5"));
    ...
    TestUtils.waitForCondition(new Function<Void, Boolean>() {
      ...
      public Boolean apply(@Nullable Void aVoid) {
        try {
          ...
          File tableDataDir = new File(CommonConstants.Server.DEFAULT_INSTANCE_DATA_DIR + "-0", offlineTableName);
          ...
          for (File indexDir : indexDirs) {
            ...
            for (String metricName : segmentMetadata.getSchema().getMetricNames()) {
              if (segmentMetadata.hasDictionary(metricName)) {
                return false;
              }
            }
          }
