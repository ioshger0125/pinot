...
public class ConvertToRawIndexTaskGenerator implements PinotTaskGenerator {
  ...
  public List<PinotTaskConfig> generateTasks(@Nonnull List<TableConfig> tableConfigs) {
    ...
    for (TableConfig tableConfig : tableConfigs) {
      ...
      Map<String, String> taskTypeConfigs =
          tableTaskConfig.getTaskTypeConfigs(MinionConstants.ConvertToRawIndexTask.TASK_TYPE);
      int tableMaxNumTasks;
      try {
        tableMaxNumTasks = Integer.valueOf(taskTypeConfigs.get(MinionConstants.TABLE_MAX_NUM_TASKS_KEY));
      } catch (Exception e) {
      ...
      for (OfflineSegmentZKMetadata offlineSegmentZKMetadata : _clusterInfoProvider.getOfflineSegmentsMetadata(
          offlineTableName)) {
        // Submit up to tableMaxNumTasks tasks each time for each table
